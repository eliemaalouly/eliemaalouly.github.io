...
$(function () {
  ...
  // Define mobile breakpoint - below this all items go to hamburger menu
  var mobileBreakpoint = 500; // in pixels
  ...
  function check() {
    ...
    // If we're below the mobile breakpoint, move all items to hamburger
    if (winWidth <= mobileBreakpoint) {
      // Move all visible items to hidden
    ...
    // Add extra margin to ensure links don't wrap
    ...
    if (requiredSpace > availableSpace) {
      ...
      // Only move items from hidden to visible if we're not at the mobile breakpoint
      if (numOfVisibleItems < breakWidths.length && $hlinks.children().length > 0) {
